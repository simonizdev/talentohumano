...
<h3>Reporte ausencias de empleados</h3>
...
<div class="row">
    <div class="col-sm-4">
      <div class="form-group">
        <?php echo $form->error($model,'motivo_ausencia', array('class' => 'pull-right badge bg-red')); ?>
        <?php echo $form->label($model,'motivo_ausencia'); ?>
        <?php
            $this->widget('ext.select2.ESelect2',array(
                'name'=>'Reporte[motivo_ausencia]',
                'id'=>'Reporte_motivo_ausencia',
...
function reporte_pantalla(){
  ...
  var motivo_ausencia = $("#Reporte_motivo_ausencia").val();
  ...
  $.ajax({ 
    type: "POST", 
    url: "<?php echo Yii::app()->createUrl('reporte/ausenciaspant'); ?>",
    data: data,
...
function resetfields(){
  ...
  $('#Reporte_motivo_ausencia').val('').trigger('change');
